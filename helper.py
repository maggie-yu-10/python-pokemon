...
def player_attack(lb, text, damage, health, event=None):
    lb.config(text=text)
    lb.after(INTERVAL * 3, lambda: lb.config(text='Gutmann PH -%d' % damage))  # 1000ms
    lb.after(INTERVAL * 2, lambda: health.config(value=max(0, health['value'] - damage)))


def enemy_attack(lb, health, event=None):

    t1 = ['Amy move: u failed all your classes',
          'Amy move: u are forced to transfer to college',
          'Amy move: u got kicked out']

    t2 = ['Your GPA is now 1: your   PH   -30',
          'You can no longer be Wharton Snake: your PH -20',
          'Kicked out of Penn?? you die']
    damages = [20, 30, float('inf')]

    n = random.randint(0, len(t1) - 1)
    lb.after(INTERVAL * 2, lambda: lb.config(text=t1[n]))
    lb.after(INTERVAL * 4, lambda: lb.config(text=t2[n]))
    lb.after(INTERVAL * 4, lambda: health.config(value=max(0, health['value'] - damages[n])))
...
def one_turn(lb, text, damage, hp1, hp2, end_frame, event=None):
    ...
    lb.after(INTERVAL * 4, lambda: is_end(hp1, hp2, end_frame))
